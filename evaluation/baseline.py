    ...
    for key in query_keys:
        ...
        for q in queries:
            ...
            for msg_id, msg in q.items():
                ...
                is_engage = (msg.get("count_num",0) > 0) or (msg.get("hover_count",0) > 0)
                if is_engage:
                    engagements += 1
                ...
                navigate_count += len(navigate_details)  
...
df.to_csv("C:/Users/Administrator/Desktop/cope/COPE/evaluation/results/baseline_result.csv", index=False)
print(f"指标计算完成，共处理 {len(all_session_metrics)} 个会话，已保存到 baseline_result.csv")
